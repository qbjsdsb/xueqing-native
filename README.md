...
Backup + private Storage Restore Gate 已完成并合入主线：provider-portable PostgreSQL logical archive、private Attachment bytes + SHA-256 manifest、独立加密归档与 fresh isolated restore 均已通过；恢复后 application-owned identity、teacher handoff、Case/Action lifecycle、revoked identity、operation replay/idempotency 与 sent Invitation Delivery 语义保持一致，并记录了受控虚构 rehearsal 的实测 backup/restore 时间。当前唯一自动执行线转为 **Production Client Deployment Profile + Session/Auth (#75)**：让 Windows / Android release 客户端安全连接已接受的 Supabase Singapore 生产拓扑，同时保持 IdentityLink、环境隔离、Durable Intent 与 provider-neutral Domain 边界；#75 收口后先进入 **Windows Agent Readiness (#79)**，冻结 UI Automation、键盘、deep link 与必要的薄型本地 Agent 命令面，再做 CloudBase second-provider conformance、Signing/Recovery 和 V1 RC。任何环境始终只有一个 authoritative provider，不做 active-active 或 live dual-write。
