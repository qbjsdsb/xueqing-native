...
[TestClass]
public sealed class CreateObservationCommandTests
{
    private static readonly Guid ActorId = Guid.Parse("10000000-0000-0000-0000-000000000001");
    private static readonly Guid OperationId = Guid.Parse("74000000-0000-0000-0000-000000000001");
    ...
    public async Task Command_sends_only_domain_intent_and_accepts_matching_receipt()
    {
        ...
        Assert.AreEqual(ObservationId, result.Receipt.ObservationId);
        Assert.AreEqual(ActorId, result.Receipt.ActorAppUserId);
        Assert.AreEqual("Bearer access-token", handler.LastAuthorization);
        Assert.AreEqual("publishable-key", handler.LastApiKey);

        using var body = JsonDocument.Parse(
            handler.LastBody ?? throw new AssertFailedException("Missing command body."));
        var root = body.RootElement;
        Assert.AreEqual(OperationId.ToString("D"), root.GetProperty("p_operation_id").GetString());
        Assert.AreEqual(AssignmentId.ToString("D"), root.GetProperty("p_assignment_id").GetString());
        Assert.AreEqual("windows_quick_capture", root.GetProperty("p_client_capture_metadata").GetProperty("source").GetString());
        Assert.IsFalse(root.TryGetProperty("actor_app_user_id", out _));
        Assert.IsFalse(root.TryGetProperty("p_actor_app_user_id", out _));
    }

    [TestMethod]
    public async Task Command_fails_closed_when_receipt_actor_or_scope_does_not_match()
    {
        var cases = new[]
        {
            ValidReceipt(operationId: Guid.Parse("74000000-0000-0000-0000-000000000009")),
            ...
            ValidReceipt(subjectProfileId: Guid.Parse("40000000-0000-0000-0000-000000000009")),
        };
        ...
        foreach (var receipt in cases)
        {
            ...
            Assert.AreEqual(CreateObservationFailureKind.ResultUnknown, result.Failure?.Kind);
    ...
    public async Task Command_maps_stable_domain_failures_without_provider_types()
    {
        ...
        var validation = await CommandWithFailure("XQ_INVALID_OBSERVATION_TEXT");
        ...
        var unknown = await CommandWithFailure(null, HttpStatusCode.ServiceUnavailable);
        var transient = await CommandWithFailure(null, HttpStatusCode.TooManyRequests);

        Assert.AreEqual(CreateObservationFailureKind.AuthorityChanged, authority.Failure?.Kind);
        Assert.AreEqual(CreateObservationFailureKind.Validation, validation.Failure?.Kind);
        Assert.AreEqual(CreateObservationFailureKind.OperationConflict, conflict.Failure?.Kind);
        Assert.AreEqual(CreateObservationFailureKind.AuthenticationRequired, auth.Failure?.Kind);
        Assert.AreEqual(CreateObservationFailureKind.ResultUnknown, unknown.Failure?.Kind);
        Assert.AreEqual(CreateObservationFailureKind.Transient, transient.Failure?.Kind);
    ...
    public async Task Command_validates_intent_before_network()
    {
        ...
        var missingOperation = await command.ExecuteAsync(
            ...
            ActorId);
        var blankText = await command.ExecuteAsync(
            Request() with { RawText = "   " },
            ActorId);
        var longText = await command.ExecuteAsync(
            Request() with { RawText = new string('a', 10_001) },
            ActorId);
        var missingAssignment = await command.ExecuteAsync(
            Request() with { AssignmentId = Guid.Empty },
            ActorId);

        Assert.AreEqual(CreateObservationFailureKind.Validation, missingOperation.Failure?.Kind);
        Assert.AreEqual(CreateObservationFailureKind.Validation, blankText.Failure?.Kind);
        Assert.AreEqual(CreateObservationFailureKind.Validation, longText.Failure?.Kind);
        Assert.AreEqual(CreateObservationFailureKind.Validation, missingAssignment.Failure?.Kind);
    ...
    public async Task Command_does_not_send_without_live_access_token()
    {
        ...
        Assert.AreEqual(CreateObservationFailureKind.AuthenticationRequired, result.Failure?.Kind);
        Assert.AreEqual(0, handler.RequestCount);
    }
    ...
    public void Command_rejects_non_loopback_plain_http()
    {
        Assert.ThrowsExactly<ArgumentException>(
            () => new PostgrestCreateObservationCommand(
    ...
    private static async Task<CreateObservationResult> CommandWithFailure(
        string? message,
    ...
    private static PostgrestCreateObservationCommand CreateCommand(RecordingHandler handler) =>
        new(
            new HttpClient(handler),
            new Uri("https://example.supabase.co/"),
            "publishable-key",
    ...
    private static string ValidReceipt(
        ...
        Guid? actorId = null,
        Guid? studentId = null,
        Guid? subjectProfileId = null) =>
        JsonSerializer.Serialize(new
        {
            command = "create_observation_v1",
            operation_id = operationId ?? OperationId,
            observation_id = ObservationId,
            actor_app_user_id = actorId ?? ActorId,
            organization_id = OrganizationId,
            student_id = studentId ?? StudentId,
            subject_profile_id = subjectProfileId ?? SubjectProfileId,
            subject_key = "chinese",
            server_committed_at = DateTimeOffset.Parse("2026-09-22T10:01:00Z"),
        });
